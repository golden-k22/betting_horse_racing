...
import betfair_bet_api
import betfair_live_api
...
def requestRaceDetails(race, stake_val):
    ...
    runners_lay, market_id = betfair_live_api.get_live_runner_detail(race['MeetingName'], race_name_key)
    if runners_lay.empty:
        ...
        for index, runner in enumerate(runnersDetail):
            if runners_lay['Lay Size'].count() - 1 > index:
                ...
                lays = runners_lay[runners_lay['Runner Name'].astype(str).str.lower().str.contains(runner['RunnerName'].lower())]
                # lays = sel_lays.iloc[index]
...
def calc_stake(stake_var, back, lays):
    ...
    bbr = 0
    profit = 0
    if len(lays['Lay Price'])>0:
        lay_prices = lays['Lay Price'].values[0]
        if len(lays['Lay Size'])>0:
            lay_sizes=lays['Lay Size'].values[0]
            l_size = 0
            for l_index, price in enumerate(lay_prices):
                print("---price----------", price)
                temp_stake = ((back - 1) / (price - 0.05)) * stake_var
                l_size = l_size + lay_sizes[l_index]
...
class App(tk.Tk):
    ...
    def displayDetailList(self, index, race, horse_name, back, lay, stake, bbr):
        ...
        is_placed=self.is_bet_placed(race['MeetingName'],race['RaceType'],race['RaceNo'],horse_name)
        actionBtn = tk.Button(self.r_detail_frame, text="Bet", width=12)
        if self.isFloat(stake) == False or self.is_bonus_available() == False or is_placed:
    ...
    def bet(self, param):
        ...
        if self.isFloat(self.stake_entry.get()):
            stake_val = float(self.stake_entry.get())
            race_name_key = "R" + str(race["RaceNo"]) + " " + str(race["Distance"]) + "m"
            runnerStake = float(runner['Stake'])
            runnerLay = float(runner['Lay'])
            betfair_result = betfair_bet_api.place_order(race['MeetingName'], race_name_key, runner['RunnerName'], runnerStake,
                                                         runnerLay)
            if betfair_result:
                ...
                tab_browser.place_bonus_bet(date_str, meeting_str, venue_str, race_type, race_no, runner_no, stake_val,
                                            is_fixed_bet)
                self.update_history_after_bet(meeting_str, race_type, race_no, runner['RunnerName'])
                self.show_detail(self.selectedRaceId)
                self.setBonusBets()
    ...
    def is_bet_placed(self, meetingName, raceType, raceNo, runnerName):
        result=False
        matches = [r for r in self.history if (
                r['MeetingName']==meetingName and
                r['RaceType']== raceType and
                r['RaceNo']==int(raceNo)
        )]
        if len(matches)>0:
            matched_runner=[h for h in matches[0]['Runners'] if h==runnerName]
            if len(matched_runner)>0:
                result = True
        return result

    def update_history_after_bet(self, meetingName, raceType, raceNo, runnerName):

        matches = [r for r in self.history if (
                r['MeetingName'] == meetingName and
                r['RaceType'] == raceType and
                int(r['RaceNo']) == int(raceNo)
        )]
        if len(matches)>0:
            matches[0]['Runners'].append(runnerName)
        else:
            new_history_item={'MeetingName': meetingName, 'RaceType': raceType, 'RaceNo': int(raceNo), 'Runners': [runnerName]}
            self.history.append(new_history_item)

        with open('placed_bets.json', 'w') as outfile:
            json.dump(self.history, outfile, indent=4)

    def update_history_from_race(self):
        temp_history = copy.deepcopy(self.history)
        for i, hist_race in enumerate(temp_history):
            matches = [r for r in races if (
                        hist_race['MeetingName'] == r['MeetingName'] and hist_race['RaceType'] == r['RaceType'] and
                        hist_race['RaceNo'] == r['RaceNo'])]
            if len(matches) == 0:
                self.history.remove(hist_race)
        with open('placed_bets.json', 'w') as outfile:
            json.dump(self.history, outfile, indent=4)
        ...
        for i, race in enumerate(races, start=0):
            ...
            if time_difference.total_seconds() > 0:
                ...
                if "MeetingName" in race.keys():
                    if race['Location'] in ['VIC', 'QLD', 'TAS', 'WA', 'SA', 'NT', 'ACT', 'NSW']:
    ...
    #
    #                 if self.isDigit(self.stake_entry.get()):
    ...
    def periodicalFunc(self, selectedId):
        ...
        if self.isFloat(self.stake_entry.get()):
            ...
            self.clearDetailFrame()
            max_horse = 0
            if self.isDigit(self.maxh_entry.get()):
            ...
            for i, runner in enumerate(runners, start=0):
                if (runner['bettingStatus'] == "Open"):
                    if "RunnerName" in runner.keys():
                        is_available = 1
                        if self.isDigit(self.maxO_entry.get()):
                            max_odds = int(self.maxO_entry.get())
                            if int(runner['ReturnWin']) >= max_odds:
                                is_available = 0
                        if self.isDigit(self.minO_entry.get()):
                            min_odds = int(self.minO_entry.get())
                            if int(runner['ReturnWin']) < min_odds:
                                is_available = 0
                        if is_available == 1:
                            if max_horse == 0:
                                self.displayDetailList(i, races[selectedId], runner['RunnerName'], runner['ReturnWin'],
...
if __name__ == '__main__':
    ...
    tab_browser = TAB()
    my_file = Path("placed_bets.json")
    history = []
    if my_file.exists():
        with open('placed_bets.json', 'r') as file:
            history_json = file.read().replace('\n', '')
        history = json.loads(history_json)
    App(history).mainloop()
