...
def bet_flat(df, stake = 1, back_odds = 'market_odds', lay_odds = 'market_odds'):
    ...
    """
    df_fav=df.groupby('market_id')['best_back_3m']
    df=df.assign(fav=df_fav.transform(min))

    df['bet_side'] = np.where((df[back_odds]==df['fav']),
                        "B",
                       "P"
                       )
    ...
    df.to_csv("df_commission.csv", encoding='utf-8')
...
def bet_eval_metrics(d, side = False):
    ...
    metrics.to_csv("df_metric.csv", encoding='utf-8')
...
dateDFList = []
dateList = pd.date_range(date(2022,6,25),date(2022,6,30),freq='d')
...
bets = bet_flat(df, stake = 50, back_odds = 'best_back_3m', lay_odds = 'best_back_3m')
